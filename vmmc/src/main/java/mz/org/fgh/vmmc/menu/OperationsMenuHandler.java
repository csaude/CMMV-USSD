...
public class OperationsMenuHandler implements MenuHandler {
       ...
       public String handleMenu(UssdRequest ussdRequest, CurrentState currentState, MenuService menuService, OperationMetadataService operationMetadataService,
	     ...
	     if (currentMenu != null) {
			 ...
			 if (currentMenu.getCode().equalsIgnoreCase(ConstantUtils.MENU_AUTHENTICATION_CODE)) {
			        ...
			        if (mapClinics.containsKey(ussdRequest.getText())) {
				      ...
				      return MessageFormat.format(ConstantUtils.MESSAGE_OPCAO_INVALIDA,
						  StringUtils.remove(getClinicsByDistrictMenu(ussdRequest, currentState, sessionDataService, currentMenu), "CON "));
			        }
			 } else if (ussdRequest.getText().equalsIgnoreCase("1")
				      && ConstantUtils.MENU_CONFIRMATION_SMS_CLINICS_LIST_CODE.equalsIgnoreCase(currentMenu.getCode())) {
			        // TODO: Enviar sms com a lista de clinicas
			        long districtId = Long.parseLong(sessionDataService.findByCurrentStateIdAndAttrName(currentState.getId(), "districtId").getAttrValue());
			        List<Clinic> clinics = RestClient.getInstance().getClinicsByDistrict(districtId).getClinics().stream()
			        ...
			        int day = Integer.parseInt(operationMetadataService.getMetadatasByOperationTypeAndSessionId(currentState.getId(), currentState.getLocation())
					    .get("appointmentDay").getAttrValue());
			        int month = StringUtils.isNotBlank(ussdRequest.getText()) ? Integer.parseInt(ussdRequest.getText())
					    : Integer.parseInt(
							operationMetadataService.getMetadatasByOperationTypeAndSessionId(currentState.getId(), currentState.getLocation())
								     .get("appointmentMonth").getAttrValue());
			        if (!DateUtils.isValidDate(day, month)) {
				      return MessageFormat.format(ConstantUtils.MESSAGE_OPCAO_INVALIDA,
						  StringUtils.remove(MessageFormat.format(MessageUtils.getMenuText(currentMenu), DateUtils.getAppointmentsMonth()), "CON"));
			        }
		   ...
		   return navegate(currentMenu, ussdRequest, currentState, menuService, operationMetadataService, sessionDataService, infoMessageService);

	     } else {

		   return MainMenuHandler.getInstance().handleMenu(ussdRequest, currentState, menuService, operationMetadataService, sessionDataService, infoMessageService);
       ...
       public String recoverSession(UssdRequest request, CurrentState currentState, MenuService menuService, SessionDataService sessionDataService) {
	     ...
	     if (ConstantUtils.MENU_CLINICS_LIST_CODE.equalsIgnoreCase(menu.getCode()) || ConstantUtils.MENU_CLINICS_LIST_APPOINTMENT_CODE.equalsIgnoreCase(menu.getCode())) {
       ...
       private String getClinicsByDistrictMenu(UssdRequest request, CurrentState currentState, SessionDataService sessionDataService, Menu menu) {
	     long districtId = Long.parseLong(sessionDataService.findByCurrentStateIdAndAttrName(currentState.getId(), "districtId").getAttrValue());
       ...
       private String navegate(Menu currentMenu, UssdRequest request, CurrentState currentState, MenuService menuService, OperationMetadataService operationMetadataService,
	     ...
	     if ((currentMenu.getMenuItems().size() > 1) || (currentMenu.getMenuItems().size() == 1 && StringUtils.trim(request.getText()).equals("0"))) {
		   ...
		   if (currentMenu.getCode().equalsIgnoreCase(ConstantUtils.MENU_CLINICS_LIST_APPOINTMENT_CODE) && !request.getText().equalsIgnoreCase("#")
		   ...
		   if (nextMenu.getCode().equalsIgnoreCase(ConstantUtils.MENU_APPOINTMENT_CONFIRMATION_CODE)) {
			 ...
			 currentState.setIdMenu(nextMenu.getId());
			 menuService.saveCurrentState(currentState);
			 appointmentRequest = operationMetadataService.createAppointmentRequestByMetadatas(request, currentState.getLocation(), currentState);
			 appointmentRequest.setClinic(Long.parseLong(sessionDataService.findByCurrentStateIdAndAttrName(currentState.getId(), "clinicId").getAttrValue()));
			 appointmentRequest.setUtente(Long.parseLong(sessionDataService.findByCurrentStateIdAndAttrName(currentState.getId(), "utenteId").getAttrValue()));
			 appointmentRequest.setClinicName(sessionDataService.findByCurrentStateIdAndAttrName(currentState.getId(), "clinicName").getAttrValue());
			 ...
			 return MessageFormat.format(getNextMenuText(currentState, currentMenu, menuService), DateUtils.getAppointmentsMonth());
       ...
       private String getAppointmentDetails(CurrentState currentState, MenuService menuService, SessionDataService sessionDataService, Menu nextMenu) {

	     String utenteId = sessionDataService.findByCurrentStateIdAndAttrName(currentState.getId(), "utenteId").getAttrValue();
	     AppointmentSearchResponse app = RestClient.getInstance().getAppointmentByUtenteId(utenteId);
	     if (app.getStatusCode() == 200) {

		   String details = MessageFormat.format(ConstantUtils.MESSAGE_APPOINTMENT_DETAILS, DateUtils.getSimpleDateFormat(app.getAppointmentDate()),
			        app.getClinic().getName(), app.getStatus());
       ...
       private String autenticate(Menu currentMenu, UssdRequest request, CurrentState currentState, MenuService menuService, SessionDataService sessionDataService) {
	     ...
	     if (response.getStatusCode() == 200 && phoneNumber.equalsIgnoreCase(response.getCellNumber())) {
		   ...
		   return MessageFormat.format(ConstantUtils.MESSAGE_LOGIN_FAILED, StringUtils.remove(MessageUtils.getMenuText(currentMenu), "CON "));
	     }
       }
       ...
       private String getClinicsByDistrictId(long districtId, UssdRequest ussdRequest) {
	     ...
	     if (ussdRequest == null || !ussdRequest.getText().equalsIgnoreCase("#")) {
		   clinicsList = RestClient.getInstance().getClinicsByDistrict(districtId).getClinics().stream().sorted(Comparator.comparing(Clinic::getName))
		   ...
		   String menuText = getClinicsMenu(clinicsList.subList(0, pagingSize > clinicsList.size() ? clinicsList.size() : pagingSize));
